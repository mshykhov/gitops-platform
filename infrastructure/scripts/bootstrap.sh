...
print_next() {
    ...
    echo ""
    echo "Then install ArgoCD:"
    echo ""
    echo "  kubectl create namespace argocd"
    echo "  kubectl apply -n argocd -f https://raw.githubusercontent.com/argoproj/argo-cd/stable/manifests/install.yaml"
    echo "  kubectl wait --for=condition=available deployment/argocd-server -n argocd --timeout=300s"
    echo ""
    echo "Generate SSH key for ArgoCD:"
    echo ""
    echo "  ssh-keygen -t ed25519 -C argocd -f ~/.ssh/argocd -N \"\""
    echo "  cat ~/.ssh/argocd.pub"
    echo ""
    echo "Add public key to GitHub repo → Settings → Deploy keys"
    echo "See infrastructure/README.md Step 3 for full setup"
    echo ""
}
